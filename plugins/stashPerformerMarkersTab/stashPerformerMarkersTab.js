    ...
    function performerPageHandler() {
        waitForElementClass("nav-tabs", async function (className, el) {
            ...
            if (!document.getElementById(markersTabId)) {
                ...
                // For /scenes/markers, Stash expects the filter to be nested inside scene_marker_filter
                const filterParams = {
                    scene_marker_filter: {
                        performers: {
                            value: [performerId],
                ...
                // Standard Stash encoding: JSON -> UTF-8 -> Base64
                const base64Filter = btoa(unescape(encodeURIComponent(JSON.stringify(filterParams))));
